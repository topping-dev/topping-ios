...
do
    ...
    blockingtcp.sleep      = socket.sleep --socketcore.sleep
...
do
    ...
    local LOG_LEVEL = 4 -- log only errors and warnings
...
local function functionFunc(self, value, depth, thread)
...
local function debugger_loop(self, async_packet)
    ...
    while true do
        -- reads packet
        local packet = async_packet or read_packet(self.skt)--assert(read_packet(self.skt))
        async_packet = nil
        ...
        if func then
            ...
            if not ok then -- internal exception
                ...
                if type(cont) == "table" and getmetatable(cont) == DBGP_ERR_METATABLE then
                	...
                	log("DEBUGGER", "DEBUG", "Error packet " .. packet .. " cmd:" .. cmd)
