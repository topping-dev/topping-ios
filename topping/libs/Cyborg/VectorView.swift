...
@objc(VectorView)
public class VectorView: NSView {
    ...
    @objc public var tint: AndroidTint = (.src, .clear) {
        didSet {
            updateLayers()
        }
    }
    ...
    @objc public var theme: ThemeProviding {
        didSet {
            updateLayers()
        }
    }

    @objc private let resources: ResourceProviding

    /// The drawable to display.
    @objc open var drawable: VectorDrawable? {
        didSet {
            updateLayers()
            needsLayout = true
...
public class VectorView: UIView {
    ...
    /// It is your responsibility to ensure that changes
    /// to `theme` also change `tint` if appropriate.
    @objc public var tint: AndroidTint = AndroidTint(blendMode: BlendMode.src!, color: UIColor.clear) {
        didSet {
            updateLayers()
        }
    }
    ...
    ///
    /// - parameter externalValues: A source for external values to use to theme the VectorDrawable.
    @objc public init(theme: ThemeProviding, resources: ResourceProviding) {
        self.theme = theme
        self.resources = resources
        super.init(frame: .zero)
    }
...
/// corresponds to the objects of the same name on Android. You can reimplement the
/// Android behavior, or write your own system.
@objc(ResourceProviding)
public protocol ResourceProviding : AnyObject {
...
@objc(ExternalValues)
public class ExternalValues : NSObject {
    
    let resources: ResourceProviding
    let theme: ThemeProviding
    
    init(resources: ResourceProviding, theme: ThemeProviding) {
        self.resources = resources
        self.theme = theme
    }
...
extension VectorDrawable {
    ...
    func layerRepresentation(in _: CGRect,
        ...
        return Array(
            groups
                .map { group in
                    group.createLayers(externalValues: externalValues,
