...
@objc public extension VectorDrawable {
    @objc static func createUrlObjc(from url: URL) -> VectorDrawable? {
        let result = VectorDrawable.create(from: url)
        switch result {
        case .ok(let result):
            return result
        case .error(_):
    ...
    @objc static func createDataObjc(from data: Data) -> VectorDrawable? {
        ...
        return data.withBytes(or: .error("Empty data passed.")) { (bytes: UnsafePointer<Int8>) -> Result<VectorDrawable> in
            ...
            while xmlTextReaderRead(xml) == 1 {
                ...
                if let namePointer = xmlTextReaderConstName(xml) {
                    ...
                    if type == XML_READER_TYPE_SIGNIFICANT_WHITESPACE.rawValue || type == XML_READER_TYPE_COMMENT.rawValue {
