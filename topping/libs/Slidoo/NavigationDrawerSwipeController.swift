...
@objc
public protocol NavigationDrawerSwipeControllerDelegate
{
    @objc func onPresent()
    @objc func onStateChange(state: Int)
    @objc func onDismiss()
}
...
public class NavigationDrawerSwipeController: UIPresentationController {
    ...
    private var state = -1
    @objc public static let STATE_IDLE = 0
    @objc public static let STATE_DRAGGING = 1
    @objc public static let STATE_SETTLING = 2
    private var originX: CGFloat?
    @objc public var stateDelegate: NavigationDrawerSwipeControllerDelegate?
    
    private func setStateVal(_ stateToSet: Int) {
        if(state != stateToSet) {
            state = stateToSet
            self.stateDelegate?.onStateChange(state: state)
        }
    }
...
extension NavigationDrawerSwipeController {
    ...
    private func dismiss() {
        setStateVal(NavigationDrawerSwipeController.STATE_SETTLING)
        presentedViewController.dismiss(animated: true) {
            self.setStateVal(NavigationDrawerSwipeController.STATE_IDLE)
            self.stateDelegate?.onDismiss()
        }
    }
}
...
extension NavigationDrawerSwipeController {
    ...
    @objc public func didPan(panRecognizer: UIPanGestureRecognizer, screenGestureEnabled: Bool = false) {
        ...
        case .began, .changed:
            setStateVal(NavigationDrawerSwipeController.STATE_DRAGGING)
            if originX == nil {
